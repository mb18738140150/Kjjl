...
@interface TestRewriteWrongViewController ()<UITableViewDataSource,UITableViewDelegate,UIGestureRecognizerDelegate,TestModule_MyWrongQuestionInfoProtocol,TestModule_MyWrongQuestionsListProtocol,MFoldingSectionHeaderDelegate,HYSegmentedControlDelegate>

@property (nonatomic,strong) UITableView            *tableView;
@property (nonatomic, strong) NSMutableArray *statusArray;

@property (nonatomic,strong) NSArray                *rewriteQuestionChapterInfo;// 章节题
...
{
    self.currentCourseInfoDic = [[DBManager sharedManager]getMyWrongTestCourseInfoWith:@(self.courseCategoryId) type:kDBErrorType_Mywrong];
...
{
    [SVProgressHUD show];
    [[TestManager sharedManager] didRequestTestMyWrongQuestionChapterInfoWithCategoryId:self.courseCategoryId andNotifiedObject:self];
    self.currentCourseInfoDic = [[DBManager sharedManager]getMyWrongTestCourseInfoWith:@(self.courseCategoryId) type:kDBErrorType_Mywrong];
}
...
- (void)requestSectionQuestionsWithSectionId:(int)sectionId
{
    ...
    NSDictionary * infoDic = @{kTestSectionId:@(sectionId),
                               @"category":@(cateGory)};
    [SVProgressHUD show];
    [[TestManager sharedManager] didRequestTestMyWrongQuestionListWithChapterId:infoDic andNotifiedObject:self];
}
...
- (void)addHistory
{
    NSDictionary *dic = [self.rewriteQuestionChapterInfo objectAtIndex:self.selectSection];
...
- (void)didRequestMyWrongQuestionSuccess
{
    ...
    self.rewriteQuestionChapterInfo = [[[TestManager sharedManager] getMyWrongChapterInfoArray] objectAtIndex:0];
    self.simulateArray = [[[TestManager sharedManager] getMyWrongChapterInfoArray] objectAtIndex:1];
    if (self.rewriteQuestionChapterInfo.count == 0) {
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    NSDictionary *dic = [self.rewriteQuestionChapterInfo objectAtIndex:section];
    view.isChapter = NO;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([tableView isEqual:self.simulateTableView]) {
        ...
        NSArray * dbArray = [[DBManager sharedManager] getSimulateTestInfoWith:kDBErrorType_Mywrong];
    ...
    NSDictionary *dic = [self.rewriteQuestionChapterInfo objectAtIndex:indexPath.section];
    NSArray *array = [dic objectForKey:kTestChapterSectionArray];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if ([self.simulateTableView isEqual:tableView]) {
        ...
        for (NSDictionary * simuDic in [[DBManager sharedManager] getSimulateTestInfoWith:kDBErrorType_Mywrong]) {
...
-(void)MFoldingSectionHeaderTappedAtIndex:(NSInteger)index
{
    ...
    NSDictionary *dic = [self.rewriteQuestionChapterInfo objectAtIndex:index ];
    NSArray *array = [dic objectForKey:kTestChapterSectionArray];
