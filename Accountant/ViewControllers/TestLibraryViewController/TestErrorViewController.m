...
@interface TestErrorViewController ()<UITableViewDelegate,UITableViewDataSource,TestModule_ErrorInfoProtocol,TestModule_ErrorQuestionProtocol,UIGestureRecognizerDelegate,MFoldingSectionHeaderDelegate,HYSegmentedControlDelegate>

@property (nonatomic,strong) UITableView        *tableView;
@property (nonatomic,strong) NSArray            *easyWrongArray;// 章节题
...
{
    self.currentCourseInfoDic = [[DBManager sharedManager]getMyWrongTestCourseInfoWith:@(self.courseCategoryId) type:kDBErrorType_Easywrong];
...
{
    [SVProgressHUD show];
    [[TestManager sharedManager] didRequestTestErrorInfoWithCategoryId:self.courseCategoryId andNotifiedObject:self];
    self.currentCourseInfoDic = [[DBManager sharedManager]getMyWrongTestCourseInfoWith:@(self.courseCategoryId) type:kDBErrorType_Easywrong];
}
...
- (void)requestSectionQuestionsWithSectionId:(int)sectionId
{
    ...
    [SVProgressHUD show];
    [[TestManager sharedManager] didRequestTestErrorQuestionWithSectionId:infoDic andNotifiedObject:self];
}
...
- (void)addHistory
{
    NSDictionary *dic = [self.easyWrongArray objectAtIndex:self.selectSection];
...
- (void)didReqeustErrorInfoSuccess
{
    ...
    self.easyWrongArray = [[[TestManager sharedManager] getMyWrongChapterInfoArray] objectAtIndex:0];
    self.simulateArray = [[[TestManager sharedManager] getMyWrongChapterInfoArray] objectAtIndex:1];
    if (self.easyWrongArray.count == 0) {
...
//    [self.view addSubview:self.segmentControl];
    ...
    UIView * titleView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, kScreenHeight, 37)];
    ...
    self.scrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 37, kScreenWidth, kScreenHeight - kStatusBarHeight - kNavigationBarHeight - 37)];
    [self.view addSubview:self.scrollView];
    self.scrollView.scrollEnabled = NO;
    
    CGRect simulateTableViewRect = CGRectMake(kScreenWidth, 0, kScreenWidth, kScreenHeight - kNavigationBarHeight - kStatusBarHeight - 37);
    ...
    CGRect tableViewRect = CGRectMake(0, 0, kScreenWidth, kScreenHeight - kNavigationBarHeight - kStatusBarHeight - 37);
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    NSDictionary *dic = [self.easyWrongArray objectAtIndex:section];
    view.isChapter = NO;
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView
{
    ...
    return self.easyWrongArray.count;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([tableView isEqual:self.simulateTableView]) {
        ...
        NSArray * dbArray = [[DBManager sharedManager] getSimulateTestInfoWith:kDBErrorType_Easywrong];
    ...
    NSDictionary *dic = [self.easyWrongArray objectAtIndex:indexPath.section];
    NSArray *array = [dic objectForKey:kTestChapterSectionArray];
    ...
    cell.cellType = CellType_easyWrong;
...
-(void)MFoldingSectionHeaderTappedAtIndex:(NSInteger)index
{
    ...
    NSDictionary *dic = [self.easyWrongArray objectAtIndex:index ];
    NSArray *array = [dic objectForKey:kTestChapterSectionArray];
